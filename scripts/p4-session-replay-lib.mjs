...
export function artifactPaths(runDir) {
  return {
    ...
    round11TranslateFailFast: path.join(runDir, "round11-translate-provider-failfast-pass.png"),
    round11YtFailFast: path.join(runDir, "round11-yt-script-provider-failfast-pass.png"),
    console: path.join(runDir, "round11-console.log"),
    network: path.join(runDir, "round11-network.log"),
    noiseReportJson: path.join(runDir, "round11-noise-baseline-report.json"),
    noiseReportMd: path.join(runDir, "round11-noise-baseline-report.md"),
...
export function renderSummaryMarkdown(summary, files, repoRoot) {
  const lines = [
    ...
    `- round9 /yt-script provider 成功截图: ${path.relative(repoRoot, files.round9YtProvider)}`,
    `- round10 /translate provider 成功截图: ${path.relative(repoRoot, files.round10TranslateProvider)}`,
    `- round11 /translate provider 未注入 fail-fast 截图: ${path.relative(repoRoot, files.round11TranslateFailFast)}`,
    `- round11 /yt-script provider 未注入 fail-fast 截图: ${path.relative(repoRoot, files.round11YtFailFast)}`,
