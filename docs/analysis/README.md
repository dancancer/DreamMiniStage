# Analysis Index

- 当前人工分析：`docs/analysis/sillytavern-integration-gap-2026-03.md`
- 当前 Story Agent E2E 审计：`docs/analysis/2026-05-31-sillytavern-plugin-e2e-gap-audit.md`
- 当前 Story Agent E2E 修复验证：`docs/analysis/2026-05-31-story-agent-e2e-fix-verification.md`
- 当前 Story Agent prompt 拓扑验证：`docs/analysis/2026-05-31-story-agent-prompt-topology-e2e.md`
- 当前 Story Agent model gateway 验证：`docs/analysis/2026-06-01-story-agent-model-gateway-e2e.md`
- 当前 Story Agent status fallback 验证：`docs/analysis/2026-06-01-story-agent-status-fallback-e2e.md`
- 当前 Story Agent opening status 验证：`docs/analysis/2026-06-01-story-agent-opening-status-e2e.md`
- 当前 Story Agent theater opening 验证：`docs/analysis/2026-06-01-story-agent-theater-opening-e2e.md`
- 当前 Story Agent import diagnostics 验证：`docs/analysis/2026-06-01-story-agent-import-diagnostics-e2e.md`
- 当前 Story Agent preset role 拓扑验证：`docs/analysis/2026-06-01-story-agent-preset-role-topology-e2e.md`
- 当前 Story Agent origin 折叠 UI 验证：`docs/analysis/2026-06-01-story-agent-origin-collapsible-render-e2e.md`
- 当前 Story Agent initial state 验证：`docs/analysis/2026-06-01-story-agent-initial-state-e2e.md`
- 当前 Story Agent 浏览器级状态/渲染验证：`docs/analysis/2026-06-04-story-agent-browser-state-render-e2e.md`
- 当前 Story Agent 工作总结：`docs/analysis/2026-06-01-story-agent-work-summary.md`
- 当前宿主协议：`docs/analysis/session-host-bridge/README.md`
- 当前机器报告（Markdown）：`docs/analysis/sillytavern-gap-report-latest.md`
- 当前机器报告（JSON）：`docs/analysis/sillytavern-gap-report-latest.json`

历史分析请见：`docs/archive/2026-03-05-sillytavern-gap-refresh/`
