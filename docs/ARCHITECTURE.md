...
- **持久化层**：IndexedDB (`CharacterAppDB`, version 13) + localStorage；角色头像使用 Blob store。
...
| `/character-cards` | `app/character-cards/page.tsx` | 角色卡库、PNG 导入、列表/轮播视图、编辑、删除、置顶 |
| `/story-agent-import` | `app/story-agent-import/page.tsx` | Story Agent 导入向导、blueprint 预览、会话创建入口 |
...
- `lib/story-agent/import/*`：产品导入路径，提交 blueprint、角色壳和 StorySession。
- `lib/story-agent/session/*`：持久化 blueprint/session binding，并创建 blueprint-backed session。
...
### 4.1 创建 Story Agent 会话

`/story-agent-import` -> 选择角色卡/世界书/预设/正则 -> `previewStoryAgentFromFiles()` -> `SessionBlueprint` 预览 -> `commitStoryAgentFromPreview()` -> IndexedDB 写 `story_blueprints`、`characters_record`、`sessions_record`、`story_sessions` -> 跳转 `/session?id={sessionId}`。

`useSessionStore.createSession(characterId)` 只接受带 `storyBlueprintId` 的 compiled agent character。缺少 blueprint 绑定的角色会 fail-fast，不创建无法运行的 session。

同一个 compiled agent 可以有多个会话。会话删除时会删除对应 dialogue tree，但不会删除 agent character 或 blueprint。
