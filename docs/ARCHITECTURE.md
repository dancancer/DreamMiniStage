...
4. `StorySession` runtime 组装 `PromptStack`、`WorldModule` hits、recent transcript、文本 transform 与 render state。
5. `function/dialogue/chat-streaming.ts` 根据请求选择 buffered JSON 或 SSE。
6. `finalizeStoryTurn` 提交 recent transcript、世界书激活计数器和 render state；`processPostResponseAsync` 只回填 assistant response 并刷新会话摘要缓存。
