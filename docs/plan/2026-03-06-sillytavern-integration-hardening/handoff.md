...
- M1 / M2 / M3 已全部落地：
  ...
  - M3：`scripts/p4-session-replay-e2e.mjs` 已扩到 round9，当前覆盖 `/floor-teleport` 宿主锚点滚动、`/proxy` 成功切换、`/yt-script` provider 成功路径。
- 本轮（加固增量）已完成：
...
- Replay 回归现状：
  - 最新通过 run：`p4r12-1772853899670`。
  - 产物目录：`docs/plan/2026-03-03-sillytavern-gap-reduction/artifacts/p4-session-replay-p4r12-1772853899670`。
...
2. round9 已完成代理/转录成功路径后，建议 round10 继续补 `translate` provider 成功回放，形成 extension 侧双命令守卫闭环。
