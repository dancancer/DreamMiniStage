...
- `ApiCallContext` Slash 注入位已扩展为 ``；`slash-context-adapter.ts` 会统一透传到 Slash 执行上下文，并在宿主未注入时为 `setChatDisplayMode/bgcol/buttons/removePromptInjections/closechat/custom-stop-strings/model` 提供单路径默认实现（stop/model 默认落 localStorage），仍保持参数/返回值异常显式 fail-fast。
