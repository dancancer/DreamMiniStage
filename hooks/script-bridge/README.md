...
- Phase 5 的总矩阵说明书现已写入 `docs/plan/2026-03-08-sillytavern-product-roadmap/phase5-host-capability-matrix.md`；需要判断“默认支持 / 条件支持 / 显式不支持”时，以该文档和 `host-capability-matrix.ts` 为准。
...
- `ApiCallContext` Slash 注入位已扩展为 ``，并新增 `hostCapabilitySources` 供 host-debug 记录真实宿主来源；Quick Reply 透传位已补齐第二/三批命令所需的 `onToggle/Add/Remove(Global|Chat)QuickReplySet/onListQuickReplySets/onUpdateQuickReply/onAdd/Remove/ClearQuickReplyContextSet(s)/onCreate|onUpdate|onDeleteQuickReplySet`；`slash-context-adapter.ts` 会统一透传到 Slash 执行上下文，`useScriptBridge.ts` 与 `CharacterChatPanel.tsx` 现已同步转发 `tempchat/jump/translate/timed-effect/proxy/yt-script/clipboard/extension-state/gallery` 所需注入位，并在宿主未注入时只为 `setChatDisplayMode/bgcol/buttons/popup/pick-icon/is-mobile/removePromptInjections/closechat/custom-stop-strings/model` 提供单路径默认实现，避免 extension-state 再次偷偷回退到隐式 `window.pluginRegistry`。
