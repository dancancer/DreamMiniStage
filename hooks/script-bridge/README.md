...
- `ApiCallContext` Slash 注入位已扩展为 ``；Quick Reply 透传位已补齐第二/三批命令所需的 `onToggle/Add/Remove(Global|Chat)QuickReplySet/onListQuickReplySets/onUpdateQuickReply/onAdd/Remove/ClearQuickReplyContextSet(s)/onCreate|onUpdate|onDeleteQuickReplySet`；`slash-context-adapter.ts` 会统一透传到 Slash 执行上下文，并在宿主未注入时为 `setChatDisplayMode/bgcol/buttons/popup/pick-icon/is-mobile/removePromptInjections/closechat/custom-stop-strings/model` 提供单路径默认实现（stop/model 默认落 localStorage），仍保持参数/返回值异常显式 fail-fast。
