...
export interface StreamResponseEvent {
  type: "content" | "reasoning" | "complete" | "error";
  content?: string;
 ...
 * 创建流式响应构建器
 *
 * 好品味：封装流式响应的构建逻辑，保持 API 简洁
 */
export function createStreamResponseBuilder(): {
  ...
  const write = (event: StreamResponseEvent) => {
    if (controller) {
  ...
  return {
    stream,
    writer: {
      writeContent: (content: string) => write({ type: "content", content }),
      writeReasoning: (reasoning: string) => write({ type: "reasoning", thinkingContent: reasoning }),
      writeComplete: (result) => {
        write({
          type: "complete",
      ...
      writeError: (error: string) => write({ type: "error", error }),
