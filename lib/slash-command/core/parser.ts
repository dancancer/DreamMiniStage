/**
 * Slash 内核解析器：支持管道、闭包、parser-flag、breakpoint、scopeDepth 元数据。
 */
import type { ParserFlagName, ParserFlags } from "../types";
import type {
  AstNode,
  BlockArgument,
  CommandNode,
  IfNode,
  KernelParseResult,
  Script,
  TimesNode,
  WhileNode,
} from "./types";
export interface ParseKernelOptions {
  parserFlags?: Partial<ParserFlags>;
  debugEnabled?: boolean;
}
type Token = WordToken | BlockToken;
type ParseResult<T> = ParseOk<T> | ParseErr;
interface ParseState {
  flags: ParserFlags;
  debugEnabled: boolean;
}
interface ParseContext {
  state: ParseState;
  scopeDepth: number;
}
interface WordToken {
  kind: "word";
  value: string;
  raw: string;
}
interface BlockToken {
  kind: "block";
  value: string;
  raw: string;
}
interface ParseOk<T> {
  ok: true;
  value: T;
}
interface ParseErr {
  ok: false;
  error: string;
}
const DEFAULT_PARSER_FLAGS: ParserFlags = {
  STRICT_ESCAPING: false,
  REPLACE_GETVAR: false,
};
const PARSER_FLAG_SET = new Set<ParserFlagName>(["STRICT_ESCAPING", "REPLACE_GETVAR"]);
const PARSER_FLAG_TRUE_STATES = new Set(["1", "true", "on", "yes"]);
const PARSER_FLAG_FALSE_STATES = new Set(["0", "false", "off", "no"]);
export function parseKernelScript(
  input: string,
  options: ParseKernelOptions = {},
): KernelParseResult {
  const state = createParseState(options);
  return parseKernelScriptInternal(input, state, 0);
}
function parseKernelScriptInternal(
  input: string,
  state: ParseState,
  scopeDepth: number,
): KernelParseResult {
  const trimmed = input.trim();
  if (!trimmed) {
    return { script: [], isError: true, errorMessage: "Empty command string" };
  }
  const segments = splitTopLevel(trimmed, "|");
  const script: Script = [];
  for (let i = 0; i < segments.length; i++) {
    const segment = segments[i].trim();
    if (!segment) continue;
    const node = parseSegment(segment, { state, scopeDepth });
    if (!node.ok) {
      return {
        script: [],
        isError: true,
        errorMessage: `Parse error at segment ${i + 1}: ${node.error}`,
      };
    }
    if (node.value) script.push(node.value);
  }
  if (script.length === 0) {
    return { script: [], isError: true, errorMessage: "No valid commands found" };
  }
  return { script, isError: false };
}
function createParseState(options: ParseKernelOptions): ParseState {
  return {
    flags: {
      STRICT_ESCAPING: options.parserFlags?.STRICT_ESCAPING ?? DEFAULT_PARSER_FLAGS.STRICT_ESCAPING,
      REPLACE_GETVAR: options.parserFlags?.REPLACE_GETVAR ?? DEFAULT_PARSER_FLAGS.REPLACE_GETVAR,
    },
    debugEnabled: Boolean(options.debugEnabled),
  };
}
function cloneParserFlags(flags: ParserFlags): ParserFlags {
  return {
    STRICT_ESCAPING: flags.STRICT_ESCAPING,
    REPLACE_GETVAR: flags.REPLACE_GETVAR,
  };
}
function parseSegment(segment: string, context: ParseContext): ParseResult<AstNode | null> {
  const tokenized = tokenize(segment, context.state.flags.STRICT_ESCAPING);
  if (!tokenized.ok) return { ok: false, error: tokenized.error };
  const tokens = tokenized.value;
  if (tokens.length === 0) return { ok: false, error: "Empty segment" };
  const head = tokens[0];
  if (head.kind === "block" && tokens.length === 1) {
    const parsed = parseKernelScriptInternal(head.value, context.state, context.scopeDepth + 1);
    if (parsed.isError) return { ok: false, error: parsed.errorMessage ?? "Invalid block" };
    return { ok: true, value: { type: "block", body: parsed.script, raw: segment } };
  }
  if (head.kind !== "word" || !head.value.startsWith("/")) {
    return { ok: false, error: `Invalid command head: ${head.raw}` };
  }
  const name = head.value.slice(1).toLowerCase();
  const args: string[] = [];
  const namedArgs: Record<string, string> = Object.create(null);
  const namedArgumentList: CommandNode["namedArgumentList"] = [];
  const unnamedArgumentList: CommandNode["unnamedArgumentList"] = [];
  const blocks: BlockArgument[] = [];
  for (let i = 1; i < tokens.length; i++) {
    const token = tokens[i];
    if (token.kind === "block") {
      const parsed = parseKernelScriptInternal(token.value, context.state, context.scopeDepth + 1);
      if (parsed.isError) return { ok: false, error: parsed.errorMessage ?? "Invalid block" };
      blocks.push({ type: "block", body: parsed.script, raw: token.raw });
      continue;
    }
    const namedAssignment = parseNamedArgumentToken(token, context.state.flags);
    if (namedAssignment) {
      namedArgs[namedAssignment.name] = namedAssignment.value;
      namedArgumentList.push(namedAssignment);
      continue;
    }
    const unnamedAssignment = parseUnnamedArgumentToken(token, context.state.flags);
    args.push(unnamedAssignment.value);
    unnamedArgumentList.push(unnamedAssignment);
  }
  const baseCommand: CommandNode = {
    type: "command",
    name,
    args,
    namedArgs,
    namedArgumentList,
    unnamedArgumentList,
    parserFlags: cloneParserFlags(context.state.flags),
    scopeDepth: context.scopeDepth,
    blocks,
    raw: segment,
  };
  if (baseCommand.name === "parser-flag") return applyParserFlag(baseCommand, context.state);
  if (baseCommand.name === "breakpoint") {
    if (!context.state.debugEnabled) return { ok: true, value: null };
    return { ok: true, value: { type: "breakpoint", raw: segment, scopeDepth: context.scopeDepth } };
  }
  return wrapControlNode(baseCommand);
}
function applyParserFlag(command: CommandNode, state: ParseState): ParseResult<null> {
  const rawFlagName = (
    command.args[0]
    ?? command.namedArgs.flag
    ?? command.namedArgs.name
    ?? ""
  ).trim().toUpperCase();
  if (!rawFlagName) return { ok: false, error: "Missing flag name for /parser-flag" };
  if (!isParserFlagName(rawFlagName)) return { ok: false, error: `Unknown parser flag: ${rawFlagName}` };
  const rawState = (command.args[1] ?? command.namedArgs.state ?? "on").trim().toLowerCase();
  const parsedState = parseParserFlagState(rawState);
  if (parsedState === null) return { ok: false, error: `Invalid parser flag state: ${rawState}` };
  state.flags[rawFlagName] = parsedState;
  return { ok: true, value: null };
}
function isParserFlagName(value: string): value is ParserFlagName {
  return PARSER_FLAG_SET.has(value as ParserFlagName);
}
function parseParserFlagState(rawState: string): boolean | null {
  if (PARSER_FLAG_TRUE_STATES.has(rawState)) return true;
  if (PARSER_FLAG_FALSE_STATES.has(rawState)) return false;
  return null;
}
function wrapControlNode(cmd: CommandNode): ParseResult<AstNode> {
  if (cmd.name === "if") return buildIf(cmd);
  if (cmd.name === "while") return buildWhile(cmd);
  if (cmd.name === "times") return buildTimes(cmd);
  if (cmd.name === "return") return { ok: true, value: { type: "return", value: cmd.args[0], raw: cmd.raw } };
  if (cmd.name === "break") return { ok: true, value: { type: "break", raw: cmd.raw } };
  if (cmd.name === "abort") return { ok: true, value: { type: "abort", raw: cmd.raw } };
  return { ok: true, value: cmd };
}
function buildIf(cmd: CommandNode): ParseResult<IfNode> {
  const condition = cmd.args.join(" ").trim();
  if (!condition) return { ok: false, error: "Missing condition for /if" };
  if (!cmd.blocks[0]) return { ok: false, error: "Missing then block for /if" };
  return {
    ok: true,
    value: {
      type: "if",
      condition,
      thenBlock: cmd.blocks[0].body,
      elseBlock: cmd.blocks[1]?.body,
      raw: cmd.raw,
    },
  };
}
function buildWhile(cmd: CommandNode): ParseResult<WhileNode> {
  const condition = cmd.args.join(" ").trim();
  if (!condition) return { ok: false, error: "Missing condition for /while" };
  if (!cmd.blocks[0]) return { ok: false, error: "Missing body block for /while" };
  return { ok: true, value: { type: "while", condition, body: cmd.blocks[0].body, raw: cmd.raw } };
}
function buildTimes(cmd: CommandNode): ParseResult<TimesNode> {
  if (!cmd.args[0]) return { ok: false, error: "Missing count for /times" };
  if (!cmd.blocks[0]) return { ok: false, error: "Missing body block for /times" };
  return { ok: true, value: { type: "times", count: cmd.args[0], body: cmd.blocks[0].body, raw: cmd.raw } };
}
function splitTopLevel(input: string, delimiter: string): string[] {
  const parts: string[] = [];
  let current = "";
  let inQuote = false;
  let quoteChar = "";
  let blockDepth = 0;
  for (let i = 0; i < input.length; i++) {
    const two = input.slice(i, i + 2);
    if (!inQuote && two === "{:") {
      blockDepth++;
      current += two;
      i++;
      continue;
    }
    if (!inQuote && two === ":}" && blockDepth > 0) {
      blockDepth--;
      current += two;
      i++;
      continue;
    }
    const ch = input[i];
    if (!inQuote && (ch === "\"" || ch === "'") && !isEscapedCharacter(input, i)) {
      inQuote = true;
      quoteChar = ch;
      current += ch;
      continue;
    }
    if (inQuote && ch === quoteChar && !isEscapedCharacter(input, i)) {
      inQuote = false;
      quoteChar = "";
      current += ch;
      continue;
    }
    if (!inQuote && blockDepth === 0 && ch === delimiter) {
      parts.push(current);
      current = "";
      continue;
    }
    current += ch;
  }
  if (current) parts.push(current);
  return parts;
}
function tokenize(segment: string, strictEscaping: boolean): ParseResult<Token[]> {
  const tokens: Token[] = [];
  let buffer = "";
  let inQuote = false;
  let quoteChar = "";
  let i = 0;
  const flushWord = () => {
    const value = buffer.trim();
    if (value) tokens.push({ kind: "word", value: stripQuotes(value), raw: value });
    buffer = "";
  };
  while (i < segment.length) {
    if (!inQuote && segment.startsWith("{:", i)) {
      flushWord();
      const block = readBlock(segment, i + 2, strictEscaping);
      if (!block.ok) return block;
      tokens.push({ kind: "block", value: block.value.content.trim(), raw: `{:${block.value.content}:}` });
      i = block.value.nextIndex;
      continue;
    }
    const ch = segment[i];
    if (!inQuote && (ch === "\"" || ch === "'") && !isEscapedCharacter(segment, i)) {
      inQuote = true;
      quoteChar = ch;
      buffer += ch;
      i++;
      continue;
    }
    if (inQuote && ch === quoteChar && !isEscapedCharacter(segment, i)) {
      inQuote = false;
      quoteChar = "";
      buffer += ch;
      i++;
      continue;
    }
    if (!inQuote && /\s/.test(ch)) {
      flushWord();
      i++;
      continue;
    }
    buffer += ch;
    i++;
  }
  if (strictEscaping && inQuote) {
    return { ok: false, error: "Unclosed quote under STRICT_ESCAPING" };
  }
  flushWord();
  return { ok: true, value: tokens };
}
function isEscapedCharacter(input: string, index: number): boolean {
  let slashCount = 0;
  for (let i = index - 1; i >= 0; i -= 1) {
    if (input[i] !== "\\") break;
    slashCount += 1;
  }
  return slashCount % 2 === 1;
}
function readBlock(
  segment: string,
  start: number,
  strictEscaping: boolean,
): ParseResult<{ content: string; nextIndex: number }> {
  let depth = 1;
  let i = start;
  let content = "";
  let inQuote = false;
  let quoteChar = "";
  while (i < segment.length) {
    if (!inQuote && segment.startsWith("{:", i)) {
      depth++;
      content += "{:";
      i += 2;
      continue;
    }
    if (!inQuote && segment.startsWith(":}", i)) {
      depth--;
      if (depth === 0) return { ok: true, value: { content, nextIndex: i + 2 } };
      content += ":}";
      i += 2;
      continue;
    }
    const ch = segment[i];
    if ((ch === "\"" || ch === "'") && !isEscapedCharacter(segment, i)) {
      if (!inQuote) {
        inQuote = true;
        quoteChar = ch;
      } else if (quoteChar === ch) {
        inQuote = false;
        quoteChar = "";
      }
    }
    content += ch;
    i++;
  }
  if (strictEscaping && inQuote) {
    return { ok: false, error: "Unclosed quote under STRICT_ESCAPING" };
  }
  return { ok: false, error: "Unclosed block" };
}
function stripQuotes(value: string): string {
  if (value.length < 2) return value;
  const head = value[0];
  const tail = value[value.length - 1];
  if ((head === "\"" && tail === "\"") || (head === "'" && tail === "'")) return value.slice(1, -1);
  return value;
}
function parseNamedArgumentToken(
  token: WordToken,
  parserFlags: ParserFlags,
): CommandNode["namedArgumentList"][number] | null {
  const eqRawIndex = token.raw.indexOf("=");
  if (eqRawIndex <= 0) return null;
  const name = token.raw.slice(0, eqRawIndex);
  const rawValue = token.raw.slice(eqRawIndex + 1);
  const { value, wasQuoted } = parseRawArgumentValue(rawValue, parserFlags);
  return { name, value, rawValue, wasQuoted };
}
function parseUnnamedArgumentToken(
  token: WordToken,
  parserFlags: ParserFlags,
): CommandNode["unnamedArgumentList"][number] {
  const { value, wasQuoted } = parseRawArgumentValue(token.raw, parserFlags);
  return { value, rawValue: token.raw, wasQuoted };
}
function parseRawArgumentValue(rawValue: string, parserFlags: ParserFlags): { value: string; wasQuoted: boolean } {
  let value = rawValue;
  let wasQuoted = false;
  if (rawValue.length >= 2) {
    const head = rawValue[0];
    const tail = rawValue[rawValue.length - 1];
    const quoted = (head === "\"" && tail === "\"") || (head === "'" && tail === "'");
    if (quoted) {
      value = rawValue.slice(1, -1);
      wasQuoted = true;
    }
  }
  if (parserFlags.REPLACE_GETVAR) {
    value = value
      .replace(/\{\{getvar::/gi, "{{var::")
      .replace(/\{\{getglobalvar::/gi, "{{globalvar::");
  }
  return { value, wasQuoted };
}
